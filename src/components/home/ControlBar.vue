  ...
  <div class="ControlBar">
    ...
    <div class="control r-flex">
      ...
      <div class="middle w-h r-flex">
        <span @click="playlistIndex > 0 ? prev() : ''" class="fa-pg fa fa-step-backward"></span>
        <span @click="playlist.length != 0 ? play() : ''">
          <!-- 尽量不使用v-if切换播放暂停图标，减少对dom的增加删除等操作 -->
          <i :class="isPlay ? 'fa-pg fa fa-pause' :'fa-pg fa fa-play'"></i>
        </span>
        <span @click="playlistIndex < playlist.length - 1 ? next() : ''" class="fa-pg fa fa-step-forward"></span>
      </div>
      <!-- 右 -->
...
export default {
  ...
  methods: {
    // 解构
    ...mapMutations(['changePlay', 'changePlaylistIndex', 'changeAutoPlay', 'updateDuration','changeCurrentTime']),
